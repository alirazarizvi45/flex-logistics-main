...
const CnicFrontImageControler = async (req, res) => {
  const { id } = req.body;
  try {
    ...
    await Users.update({ cnic_front: req.file.path }, { where: { id } });

    return res.status(200).json({ success: true, cnic_front: req.file.path });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
module.exports = { CnicFrontImageControler };
