...
const LicenseImageController = async (req, res) => {
  const { id } = req.body;
  try {
    ...
    await Users.update({ license_image: req.file.path }, { where: { id } });

    return res
      .status(200)
      .json({ success: true, license_image: req.file.path });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
module.exports = { LicenseImageController };
