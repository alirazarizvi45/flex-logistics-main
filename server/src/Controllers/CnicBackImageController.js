...
const CnicBackImageControler = async (req, res) => {
  const { id } = req.body;
  try {
    ...
    await Users.update({ cnic_back: req.file.path }, { where: { id } });

    return res.status(200).json({ success: true, cnic_back: req.file.path });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
module.exports = { CnicBackImageControler };
